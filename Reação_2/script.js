...
const particleCountSlider = document.getElementById('particuleCount');
const speedSlider = document.getElementById('speed');
const particuleCountValue = document.getElementById('particuleCountValue');
...
class Particle {
    ...
    update(particles){
        if(this.time >= this.delay){
            ...
            for(let other of particles){
                if(other !== this && this.time >= this.delay && other.time >= other.delay){
                    ...
                    if(distance < minDistance && distance > 0){
                        ...
                        this.angle = 2 * angleBetween - this.angle + Math.PI;
    ...
    draw(){
        if(this.time < this.time.delay) return;
        ctx.beginPath();
...
particleCountSlider.addEventListener('input', ()=>{
    particuleCountValue.textContent = particleCountSlider.value;
});
